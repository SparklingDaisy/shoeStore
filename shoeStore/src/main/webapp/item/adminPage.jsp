...
<script>
function openUpload(){
   window.open('itemUpload.jsp','itemUpload','width=800, height=580');
}
function openUpdate(){
...
<section class="adminPageSect">
   <article>
   <h2>관리자 페이지</h2>
   <div>
      <input type="button" value="상품 업로드" onclick="openUpload();" id="adminUploadButton">
   </div>
   </article>
   <article>
   <br/><br/>
   <table>
      ...
      <%
         if(userGroup!=0){
            %>
            <a href="adminPage.jsp?cp=<%=(userGroup-1)*pageSize+pageSize %>" class="groupLink">&lt;&lt;</a>
            <%
         }
         for(int i=userGroup*pageSize+1; i<=(userGroup*pageSize+pageSize); i++){
            %>
            &nbsp;&nbsp;<a href="adminPage.jsp?cp=<%=i %>" class="groupLink"><%=i %></a>&nbsp;&nbsp;
            <%
            if(i==totalPage){
               break;
            }
         ...
         if(userGroup!=(totalPage/pageSize)-(totalPage%pageSize==0?1:0)){
            %>
            <a href="adminPage.jsp?cp=<%=(userGroup+1)*pageSize+1 %>" class="groupLink">&gt;&gt;</a>
            <%
         }
         %>
            </td>
      ...
      <tbody>
         ...
         if(arr==null || arr.size()==0){
            ...
            <tr>
               ...
               <td><%=iview %></td>
               <td><input type="submit" value="수정" id="adminPageButton"></td>
               <td><a href="javascript: openDelete(<%=iidx%>,'<%=iimg%>','<%=iname%>','<%=iprice%>','<%=iview%>');"><input type="button" value="삭제" id="adminPageButton"></a></td>
            </tr>
            </form>
            <%
            }
