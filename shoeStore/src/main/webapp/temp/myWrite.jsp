					...
					<div class="my_category">
						...
						<article>
							...
							<table class="bbs_table">
								...
								<tbody>
								<%
								ArrayList<Shoe_bbsDTO> arr=bdao.bbsList(listSize,cp,(String)session.getAttribute("sid"));
								if(arr==null||arr.size()==0){
									...
									for(int i=0;i<arr.size();i++){
										...
										<tr>
											...
											<td><%if(arr.get(i).getBid().equals((String)session.getAttribute("sid"))||(String)session.getAttribute("sid")=="admin"){ %><a href="javascript:deleteB(<%=arr.get(i).getBref()%>,'<%=arr.get(i).getBid()%>','<%=arr.get(i).getBimg()%>');"><button class="black">delete</button></a>
											<%} %></td>
										</tr>
										<%
								...
								<tfoot>
									<tr>
										...
										if(userGroup!=0){
											%>
											<a href="/shoeStore/memeber/myWrite.jsp?cp=<%=(userGroup-1)*pageSize+pageSize %>">&lt;&lt;</a>
											<%
										}
										%>
										<%
										for(int i=userGroup*pageSize+1;i<=(userGroup*pageSize+pageSize);i++){
											%>&nbsp;&nbsp;<a href="/shoeStore/member/myWrite.jsp?cp=<%=i%>" style="text-decoration:none; color:black;"><%=i %></a>&nbsp;&nbsp;<%
