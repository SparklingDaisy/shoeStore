...
public class Shoe_itemDAO {
	...
	public int uploadItem(MultipartRequest mr) { 
		...
		try { 
			...
			String iview = mr.getParameter("iview").replaceAll("\r", "<br>");
	...
	public int getTotalSearchCnt(String search) {
		try {
			...
			if(search==null||search.equals("null")) {
				...
				String sql="select count(*) from shoe_item where iname like ?";
				ps=conn.prepareStatement(sql);
				ps.setString(1, "%"+search+"%");
			}
			rs = ps.executeQuery();
